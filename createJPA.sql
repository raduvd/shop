...
create table order (id bigint not null, city varchar(50), country varchar(50) not null, county varchar(50), street_address varchar(510) not null, date_of_order timestamp, customer bigint not null, shipped_from_location bigint not null, primary key (id))
create table order_detail (order_quantity integer not null, shop_order bigint not null, product bigint not null, primary key (product, shop_order))
...
alter table order add constraint FKtfp2hwv6siwe2rkt9nq9cqvst foreign key (customer) references customer
alter table order add constraint FKkbqsjxbhwkc30354cug3inxsn foreign key (shipped_from_location) references location
alter table order_detail add constraint FK6cwcb6yjgbi5fxw6488db3xii foreign key (shop_order) references order
